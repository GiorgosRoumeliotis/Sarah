...
string User::set_usrname(string new_username){
    username = new_username;
}

string User::set_password(string new_password){
    password = new_password;
}

float User::set_height(float new_height){
    height = new_height;
}

float User::set_weight(float new_weight){
    weight = new_weight;
}

int User::set_age(int new_age){
    age = new_age;
}

/*Type Male or Female*/
int User::set_gender(int new_gender){
    gender = new_gender;
}

/*Give numbers : 3, 4 or 5*/
int User::set_meals(int new_meals){
    meals = new_meals;
}

/*Give numbers 0 to 7 */
int User::set_activity(int new_activity){
    activity = new_activity;
}

/*Set it true if Vegan*/
bool User::set_vegan(bool new_vegan){
    vegan = new_vegan;
}

/*Set it true if gluten is danger*/
bool User::set_gluten(bool new_gluten_free){
    gluten_free = new_gluten_free;
}

/*Set it true if lactose is danger*/
bool User::set_lactose(bool new_lactose_free){
    lactose_free = new_lactose_free;
}

/*Set it true if Diabetic*/
bool User::set_diabetic(bool new_diabetic){
...
float User::needed_calories(int activity, int bmi, float bmr){
    ...
    if (bmi == Underweight){
        ...
        if (activity == Sedentary_Life){
            calories = 500 - 1.2*bmr;
        }
        else if (activity == Slightly_Active){
            calories = 500 - 1.375*bmr;
        }
        else if (activity == Moderate_Active){
            calories = 500 - 1.55*bmr;
        }
        else if (activity == Active_Lifestyle){
            calories = 500 - 1.725*bmr;
        }       
    }
...
// void User::load_username_info(string old_username,bool access){

//     if(access){

//         string filename = "./Users/"+old_username+"DB/UserStringInfo.txt";
//         ifstream in(filename);

//         // initialize the vector from the values in the file:
//         vector<float> strInfo{ 
//             istream_iterator<float>(in),
